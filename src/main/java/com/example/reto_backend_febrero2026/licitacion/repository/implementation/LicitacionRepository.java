...
public class LicitacionRepository implements ILicitacionRepository {
    ...
    private static final RowMapper<LicitacionModel> tenderRowMapper = (rs, rowNum) -> {
        ...
        tender.setFechaPublicacion(rs.getObject("fecha_publicacion", LocalDate.class));
    ...
    public LicitacionRepository(JdbcTemplate jdbcTemplate) {this.jdbcTemplate = jdbcTemplate;}

    @Override
    public LicitacionModel getTenderById(int tenderId) {
        String sql = "SELECT * FROM licitacion WHERE licitacion_cod = ?"; //revisar el id como se llama
        try {
            return jdbcTemplate.queryForObject(sql, tenderRowMapper, tenderId);
        }
        catch (EmptyResultDataAccessException e)
        {
            throw new RuntimeException("No se  encontro la licitacion con Id: " + tenderId);
        }
    }
